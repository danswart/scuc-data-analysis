...
subtitle: "***or...Dancing the 'TEA Twenty-Two-Step'***"
author:
...
When it comes to improving the system of education in a district this view of the data has several big problems.  One is aggregation.  It aggregates more than one rating level into a single level.  It combines the 'At Approaches' AND the 'At Meets' AND the 'At Masters' rating levels all into one combined total and calls it "At Approaches AND HIGHER Level".


To make that category useful for improvement efforts we have to break it into three separate levels so each stands on its own.  That turns out to be quite tedious and time-consuming.  Just for fun, I call this situation 'dancing the TEA Twenty-Two-Step' - because it will take at least twenty-two additional steps to make the measurements useful.  
...
Why else, then, are these reports 'ill-suited' for district improvement efforts?  

The second obstacle is TEA reports are siloed by academic year, making it nearly impossible to create a time-period-by-time-period data frame of any categories or levels the board may wish to analyze.  In fact, without time-period-by-time-period data ***no meaningful analysis is possible.***
<br>
<br>
...
In this context analysis means to subject the data ONE very simple statistical test to see if the system (or process) producing the data shows evidence of being in a stable state, with a predictable capability.  


If stable, this capability will show a range of values that constitute 'normal' for the process.  Until the process itself changes the measurements will wander between the capability limits; sometimes upward, sometimes downward, sometimes the same.  



Here is an example of what an analysis of some STAAR data looks like:
...
If the process remains the same, the proportion of students in the district taking the STAAR exam in Math that reach the level of 'Meets Approaches Standard, or Higher' will continue to fluctuate between 91% and 74%, with an average proportion of 81% (the CL).  

Any expectation of results outside those limits, without a significant jolt to the system, would not be justified.  


**This one chart contains all the information the data can reveal.  If you need or want more you will need to collect more data.**  Once the data was wrangled, it took less than 5 minutes to create the chart using a very inexpensive Excel add-in!


** Note that even the government measures imposed during the COVID-19 pandemic apparently were not a large enough jolt to the district to create a point outside the natural limits.  Or, the counter-measures employed by the district were very effective.
...
cat('</div>')
  ...
  cat('Note:  The various statistical tests you learned in Statistics 101 cannot help you when your attention is focused on the future output of your district, campus, or pedagogy.  They were designed to characterize a population as it exists at a single moment in time - how much or how many; without regard to why the population came to be the way that it is.  Even the forecasting techniques are overkill for the task at hand.')
  cat('\\end{tcolorbox}')
}
...
The crucial information for analysis of process and system capability, and the prediction of future performance, is contained in the time-period-by-time-period order.  ***The more periods included in the analysis, the stronger the evidence for your inferences from the charts.***  
...
On the other hand, analysis allows us to answer the important questions:
...
<blockquote class="my-quote">
  ...
  <p class="quote-author">          —  Elijah, to Ismael and Queequeg</p>
...
\\begin{tcolorbox}[colback=white, colframe=teal, boxrule=2pt, arc=4pt]
    ...
    \\textnormal{- Elijah, to Ismael and Queequeg} \\\\
    \\emph{ Moby Dick (1956)}
\\end{tcolorbox}
')
...
For survival, humans evolved with the ability to rapidly detect patterns.  That instinct can lead us astray when studying data.  We, quite naturally, see 'trends' and 'signals' where there are none.  Analysis was developed to help separate the signals in the data from the random fluctuations that are always present in any measurement.

Consider that for any six measurements there are at least 10 completely random arrangements that can arise.  And, we have created names for these completely artificial 'trends' that we think we see:
...
<figure>
  ...
  <figcaption>Fig:  Do you see trends or signals?</figcaption>
</figure>
...
\\begin{figure}[H] 
  ...
  \\caption{Do you see trends or signals?}
\\end{figure}
