...
***All we need are just those three things***.  All three use simple arithmetic; not even algebra.  But, none of the reports currently available have all three elements.  Usually, they have none of the three.


Accordingly, those reports cannot answer our most important questions.  We are left only with intuition and personal experience, both of which have proven to be unreliable guides for action on policy, performance, and pedagogy.


As it turns out, there is a very simple solution - **IF** we can get our data into the right form for analysis.  By reporting out results in the usual formats, neither the Texas Education Agency (TEA) nor the Texas Association of School Boards (TASB) can help us with this particular problem.  It is up to us to take control of our own reporting needs.
...
Here is an example of what such a chart looks like, using STAAR data, along with a brief interpretation of the chart:
...
* The measuremenets go up and down, but ***there are no detectable differences between them***.  The ups and downs are due to the many random forces acting on the cause system that produced the results.
...
**WHAT'S WRONG WITH WHAT WE ARE GETTING NOW?**
...
When it comes to improving the system of education in a district this view of the data has several very big problems.  One is aggregation.  The TEA reports aggregate more than one rating level into a single reporting level.  It combines the 'At Approaches Grade Level' AND the 'At Meets Grade Level' AND the 'At Masters Grade Level' ratings all into one combined total and calls it "At Approaches Grade Level Standard AND HIGHER" level.


To make that category useful for improvement efforts we have to break it into the three separate levels, so each represents a single situation.  Currently, that turns out to be quite tedious and time-consuming.  Just for fun, I call this situation 'dancing the Texas Twenty-Two-Step' - because it will take at least twenty-two additional steps to make the measurements useful!  
...
<blockquote class="my-quote">
  ...
  <p class="quote-description">  _Moby Dick (movie, 1956)_</p>
</blockquote>
')
...
Obstacle number 3 is not related to the TEA reports at all.  It has to do with how we humans are (or, as Jessica Rabbit put it in the 1988 film 'Who Framed Roger Rabbit':  "I'm not bad, I'm just drawn that way!").
