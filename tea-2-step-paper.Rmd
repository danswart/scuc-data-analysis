...
Everyone at every level in a school district receives data when trying to answering the big, important, practical questions such as:
...
*  What is the **demonstrated capability** of our district to deliver STAAR scores (or any other results) in the various categories and levels?  In other words, what is **rational** to expect from our system **in the future?**
...
***If you are a board member, district employee, or concerned citizen who asks themselves these very questions this paper can help you.  If you find little value in using data as a guide for action this paper is definitely not for you.***
...
This paper is for anyone striving to improve learning and governance in their school district.  It is directed toward school boards and superintendents, but anyone can use the simple theory and techniques to significantly improve their teaching, classrooms, campuses, and departments.  
...
Currently, there are dozens of situations where we need better answers in areas like these.  
...
knitr::kable(example_metrics_table, "simple", col.names = c("Example Metric", "Example Metric"), booktabs = TRUE, caption = "**Here are just a few examples of such challenges facing school districts today:**") %>% 
...
knitr::kable(example_metrics_table, "pipe", col.names = c("Example Metric", "Example Metric"), caption = "Here are just a few examples of such challenges facing school districts today:") %>% 
...
To make the best decisions, the ***'description-only'*** reports we currently use to evaluate the performance of programs, pedagogy, people, or anything else must come to us in a slightly different format than what we are used to seeing.  


Any evaluation of results must go beyond what is contained in 'snapshot' reports, or even reports with two-point or three-point comparisons.  As discussed later, trends cannot be established with only two or three data points.
...
The following report is called an ***Individuals-Moving Range Chart (XmR chart)***[^1] [^2] and it is an example of what a report looks like that contains the necessary information to help us.  This chart analyzes public STAAR data provided by TEA, and includes a brief interpretation of the chart.  The XmR chart provides a baseline (context) for evaluating both past and future performance.


[^1]: Technically, the chart shown is the 'Individuals' chart.  A complete XmR chart presentation also includes the 'Moving Range' chart that plots the size of the up-and-down changes between data points.  The Moving Range chart is used to calculate the range of values that is considered 'normal' for the process (i.e., the 'common cause' variation present in the process).
<br>

[^2]:  The 'XmR' chart is one of a family of 'control charts' invented and developed by Walter A. Shewhart in 1929 while an engineer at Bell Telephone Laboratories.   They are still the premier tool for understanding variation by separating 'common cause' variation from 'special cause' variation.  The XmR chart will cover nearly all situations to be found in practice in the field of education.
...
After earnestly pouring over such ***'description-only'*** reports users are left with a vague sense that a Ouija Board might be just as good!
...
It was our innate desire for context that created the 'this-month vs this-month-last-year vs year-to-date' presentation so popular today.  'This vs that' presentations are two-point comparisons that waste more resources than all the unnecessary meetings put together.
<br>
<br>
...
What makes these new reports so powerful are three unique features:
...
As it turns out, creating the analytic reports we need is very simple - **IF** we can get our data into the right form for analysis.  By reporting out results in the usual ***'description-only'*** formats, neither the Texas Education Agency (TEA) nor the Texas Association of School Boards (TASB) appear to be able to help us with this particular problem.  It is up to us to take control of our own reporting needs.
