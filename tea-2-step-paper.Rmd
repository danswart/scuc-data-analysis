...
<blockquote class="my-quote">
  <p><strong>"My aim here is to contribute something to the improvement of learning in our school district; or any school district.  My basic supposition is that school boards and administrators will welcome help to create rational policy action aimed at improving future learning in their district."</strong></p>
...
\\begin{tcolorbox}[colback=white, colframe=teal, boxrule=2pt, arc=4pt]
  \\begin{minipage}{0.95\\textwidth}
    \\raggedright
    \\large{\\textbf{"My aim here is to contribute something to the improvement of learning in our school district; or any school district. My basic supposition is that school boards and administrators will welcome help to create rational policy action aimed at improving future learning in their district."}}
  \\end{minipage}\\\\[5pt]
  \\begin{minipage}{0.95\\textwidth}
    \\raggedleft
...
Administrtation of budgets for facilities, equipment, personnel, safety and legislative compliance is a function of the managerial skill of the executive team and the school board.  It is usually well managed with thoughtful planning and careful attention to fiscal details.
...
Currently, there are dozens of situations where we need answers to questions like these.
...
To help us answer policy questions like these the reports we use to evaluate the performance of programs, pedegogy, people, or anything else must come to us in a slightly different format than what we are used to seeing.  


To be useful to us in leading our districts, any evaluation of results must provide evidence beyond 'snapshot' reports, or even two or three point comparisons.  As discussed later, trends cannot be established with only two or three data points.



\bigskip

Here is an example of what a report looks like that contains the necessary information to help us.  It analyzes public STAAR data provided by TEA, along with a brief interpretation of the chart.  It provides a baseline (context) for evaluating both past and future performance.
...
**WHAT'S WRONG WITH THE REPORTS WE USE NOW?**
...
After ernestly pouring over such reports users have a sense that a Ouija Board might be better.
...
Although this aggregation permits TEA to render abstract judgments about a district or campus, the lack of specificity renders these reports unsuitable for understanding any particular process, category or level your board may be interested in monitoring or improving.
<br>
...
Obstacle number 3 is not related to the TEA reports at all.  It has to do with our human nature (or, as Jessica Rabbit put it in the 1988 film 'Who Framed Roger Rabbit':  "I'm not bad, I'm just drawn this way!").
...
So, ***we need a report that will separate the 'common cause' variation (routine variation) from the 'special cause' variation that signals a true trend or a detectable event that changes the system.***  


Our current reports do not make that separation for us.
...
The problem arises if we react to a result that is merely random ('common-cause'  variation) as if it were a 'special cause' signal.  This is called 'tampering' and is guaranteed to make things worse.  The variation we see will increase and our good results will be diluted accordingly.  At worst, we can de-stabalize a stable system.  
...
***All we need are just those three things***; context, estimate of future performance, and analysis.  All three use simple arithmetic; not even algebra.  But, none of the reports currently available have all three elements.  Usually, they have none of the three.


Accordingly, those reports cannot answer these important questions.  We are left only with a vague intuition about what the reports we get are trying to tell us.  Intuition and personal experience have proven to be unreliable guides for action on policy, performance, and pedagogy.
...
As it turns out, creating the reports we need is very simple - **IF** we can get our data into the right form for analysis.  By reporting out results in the usual formats, neither the Texas Education Agency (TEA) nor the Texas Association of School Boards (TASB) appeaer to be able to help us with this particular problem.  It is up to us to take control of our own reporting needs.
...
<figure>
  ...
  <figcaption>Fig:  Disaggregated data taken from 5 TEA "Snapshot" Reports and 1 TAPR report</figcaption>
</figure>
...
\\begin{figure}[H]  
  ...
  \\caption{Disaggregated data taken from 5 TEA "Snapshot" Reports and 1 TAPR report}
\\end{figure}
...
However, I know from long experience that it is dangerous to rely solely on visual inspection of data that is simply displayed and not analyzed.
...
These reports were created using the disagreggated data and contain the necessary information to help us.  Again, they cover public STAAR data provided by TEA, along with a brief interpretation of the charts taken as a whole.  They provide a baseline (context) for evaluating both past and future performance in these categories and levels.
