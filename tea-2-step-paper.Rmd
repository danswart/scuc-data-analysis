...
Everyone connected to a school district seeks information that answers the questions:  "How are we doing?", "Is this a one-off, or an actual pattern?" and "Are we improving?"  Currently, there are dozens of situations where we seek answers to questions like this, but none of the reports we see can answer such questions.  We are left only with intuition and subject-matter experience to guide our actions on policy, performance, and pedagogy.
...
|On-time arrival for buses    |              |Attendance rates             |
...
To be able to answer questions like these our reports must come to us in a different format.  They must contain the context of time, in the form of plotting the results time-period-by-time-period (the more time periods presented the better). A ***very simple*** chart showing what we can rationally expect in the future should accompany the presentation of the results.
...
Here is an example of what an actual analysis of STAAR data looks like:
...
<figure>
  ...
  <figcaption>Fig:  This presentation contains all the information the data can reveal</figcaption>
</figure>
...
cat('
<div style="border: 4px solid blue; padding: 10px; border-radius: 10px;">
  <p>For those thinking about the additional work needed to provide an anlysis when reporting data to the board, or to the public, know that once the data was wrangled, ***it took less than 5 minutes to create the chart*** using a very inexpensive Excel add-in!</p>
</div>
')
...
')
  cat('
For those thinking about the additional work needed to provide an anlysis when reporting data to the board, or to the public, know that once the data was wrangled, ***it took less than 5 minutes to create the chart*** using a very inexpensive Excel add-in!
')
  cat('\\end{tcolorbox}
      ')
...
<div style="border: 4px solid red; padding: 10px; border-radius: 10px;">
  <p>Another Note:  Even the extreme government measures imposed during the COVID-19 pandemic apparently were not disruptive enough to the district to create even a point outside the natural limits.  Or, the counter-measures employed by the district were very effective.</p>
</div>
')
